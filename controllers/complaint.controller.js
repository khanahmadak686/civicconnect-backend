...
const getMyComplaints = async (req, res) => {
  try {
    const complaints = await Complaint.find({ registeredBy: req.user._id }).sort({ createdAt: -1 });
...
const upvoteComplaint = async (req, res) => {
  try {
    ...
    if (complaint.status === "RESOLVED") {
      return res.status(400).json({ success: false, message: "Cannot upvote a resolved complaint" });
    }
    const existingUpvote = await Upvote.findOne({ complaint: req.params.id, user: req.user._id });
    if (existingUpvote) {
    ...
    await User.findByIdAndUpdate(req.user._id, { $inc: { totalUpvotesGiven: 1 } });
    res.status(200).json({
      success: true,
...
const updateComplaintStatus = async (req, res) => {
  try {
    ...
    if (req.user.role === "AUTHORITY" && req.user.department !== complaint.assignedDepartment) {
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        ...
        complaint.resolutionPhotos.push({ url: result.secure_url, publicId: result.public_id });
      }
    }
...
const getDepartmentComplaints = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, total, page: parseInt(page), totalPages: Math.ceil(total / limit), stats, complaints });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
